...
class Asm:
    ...
    def kawai(self, my_code, key_name, master_key):
        # Build code data with compatibility adjustments for 3.14 CodeType signature.
        data = [
            my_code.co_argcount,
            my_code.co_posonlyargcount,
            my_code.co_kwonlyargcount,
            my_code.co_nlocals,
            my_code.co_stacksize,
            my_code.co_flags,
            key_name if master_key == 1 else my_code.co_code,
            key_name if master_key == 2 else my_code.co_consts,
            my_code.co_names,
            my_code.co_varnames,
            my_code.co_filename,
            my_code.co_name,
            my_code.co_qualname if hasattr(my_code, "co_qualname") else "<module>",
            my_code.co_firstlineno,
            my_code.co_lnotab,
            my_code.co_exceptiontable if hasattr(my_code, "co_exceptiontable") else b"",
            my_code.co_freevars,
            my_code.co_cellvars
        ]
    ...
    def get_byte_code(self, source):
        yah_kok_knok = re.findall(r"\((.*)\)", source)[0].split(",")
    ...
    def main(self):
        ...
        print("Successfully wrote to %s" % pyc_file)
...
class Dis:
    ...
    def main(self):
        ...
        print("# Don't Forget To Follow My Github Profile!")
...
def load_module(sparator):
    ...
    try:
        ...
        try:
            ...
            exit("There seems to be an error in file %s" % sparator)
    return y

def cekfile(f):
    if not os.path.exists(f):
        exit("File %s not found" % f)
